...
9. **[v2]** **Perf re-measure inputs** are listed in §0 row 5; the verification ticket owns the run (verification-strategy.md §6.1–§6.3). **Settled 2026-08-28 (#37): the harness does not use `snapshot.h`, so `snapshot` is off** — `vt_features = "-kitty_graphics,-glyph_protocol,-snapshot"` in `ghostty_pin.toml` (13.45 MB / 167 exports, the third measurement in row 4); the first prebuilt release is built with that string.
