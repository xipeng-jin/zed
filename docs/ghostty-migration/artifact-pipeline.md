...
- **Per-target verification in the same job [v2 amended — the feature decision becomes a published invariant]:**
  ...
  2. Trim invariants: **zero** `ghostty_kitty_graphics_*` exports, **zero** wuffs symbols (license-file guard, §8), **zero** `ghostty_snapshot_*` exports (snapshot trimmed since 2026-08-28, verification-strategy.md §0 / build-strategy §9 point 9). These assertions are derived from the `vt_features` input, so a deliberate change of the default updates them in the same commit.
