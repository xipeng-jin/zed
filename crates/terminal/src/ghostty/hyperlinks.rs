//! The ghostty-backed hover pipeline (SPEC.md §4.4, §6 P6): the port of
//! `alacritty/hyperlinks.rs`. Hover stays per-logical-line — OSC 8 URI walk
//! first, then URL/path regexes over one extracted logical line. Only the
//! grid layer changed: the hovered logical line is extracted into an owned
//! cell-accurate string with a byte↔cell span table, and the plain `regex`
//! crate replaces alacritty's grid-shaped `RegexSearch`. The string pipeline
//! (URL punctuation sanitization, `file://` normalization, path capture
//! handling) is ported with its logic untouched.
//!
//! Divergences from the alacritty pipeline, adjudicated in the divergence
//! ledger:
//! - ghostty exposes no OSC 8 id (G7): hover-extent equality compares URIs,
//!   so adjacent distinct links with identical URIs merge into one hover
//!   region (accepted loss, SPEC.md §9; ledger P6-001).
//! - the logical-line walk caps at [`MAX_SEARCH_LINES`] wrapped rows, the
//!   bound SPEC.md §4.4 ports from upstream alacritty's hint highlighting;
//!   Zed's alacritty fork walks uncapped, so >100-row logical lines
//!   truncate here (ledger P6-002).
//! - the URL branch's reported text comes from the same base-chars-only
//!   line the regexes run on, where alacritty re-extracts the match via
//!   `bounds_to_string` and so keeps zerowidth combining marks in the URL
//!   text (its regexes see base chars only, like ours — the
//!   `multiple_same_line` scenario depends on that; ledger P6-003).
...
use ghostty_vt::{error::Error as GhosttyError, screen::CellWide};
use log::{info, warn};
use regex::Regex;
use url::Url;
use util::ResultExt;
...
use super::{GridDimensions, PointBoundary, TerminalBackend, point_add, point_sub};
...
/// Upstream alacritty bounds hint-highlight line searches at 100 wrapped
/// rows (`MAX_SEARCH_LINES`); SPEC.md §4.4 keeps that bound here against
/// pathological fully-wrapped buffers.
...
impl RegexSearches {
    pub(crate) fn new(
        ...
        Self {
            // A case-sensitive compile matches alacritty's smart-case
            // `RegexSearch` on this pattern (its hex escapes contain
            // uppercase letters).
            url_regex: Regex::new(URL_REGEX).ok(),
...
pub(super) fn find_from_grid_point(
    ...
    let link = hyperlink_uri_at(backend, point);
    let found_word = if let Some(ref url) = link {
        // Extend across neighbors while they carry the same link. Ghostty
        // exposes no OSC 8 id (G7), so "same link" is URI equality and
        // adjacent distinct links with identical URIs merge (SPEC.md §9).
        let mut min_index = point;
        loop {
            let new_min_index = point_sub(dimensions, PointBoundary::Cursor, min_index, 1);
            if new_min_index == min_index
                || hyperlink_uri_at(backend, new_min_index).as_ref() != Some(url)
            {
                break;
            } else {
        ...
        loop {
            let new_max_index = point_add(dimensions, PointBoundary::Cursor, max_index, 1);
            if new_max_index == max_index
                || hyperlink_uri_at(backend, new_max_index).as_ref() != Some(url)
            {
                break;
            } else {
        ...
        Some((url.clone(), true, Range::new(min_index, max_index)))
    } else {
        let (line_start, line_end) = (
            line_search_left(backend, point),
            line_search_right(backend, point),
        );
        let line = extract_hover_line(backend, line_start, line_end, point)?;

        let url_match = regex_searches.url_regex.as_ref().and_then(|url_regex| {
            url_regex
                .find_iter(&line.text)
                .map(|regex_match| regex_match.range())
                .find(|range| range.contains(&line.hovered_offset))
                .and_then(|url_match| {
                    let url = line.text.get(url_match.clone())?.to_string();
                    let (url, url_match) = sanitize_url_punctuation(url, url_match);
                    let (start, end) = line.cell_range(&url_match)?;
                    Some((url, Range::new(start, end)))
                })
        });
...
fn normalize_found_word(
    found_word: (String, bool, Range),
    path_style: PathStyle,
) -> HyperlinkMatch {
    let (maybe_url_or_path, is_url, word_match) = found_word;
    normalize_hyperlink_match(maybe_url_or_path, is_url, word_match, path_style)
}
...
fn sanitize_url_punctuation(url: String, url_match: StdRange<usize>) -> (String, StdRange<usize>) {
    ...
    if chars_trimmed > 0 {
        // Every trimmed char is ASCII, so chars and bytes agree.
        let new_end = url_match.end - chars_trimmed;
        (sanitized_url, url_match.start..new_end)
    } else {
...
fn path_match(
    ...
    let hovered_point_byte_offset = line.hovered_offset;
    let trimmed_line = line.text.trim_ascii_end();
    if trimmed_line.len() <= hovered_point_byte_offset {
        return None;
    }
    let found_from_range = |path_range: StdRange<usize>,
     ...
     -> Option<(String, Range)> {
        let (link_start, link_end) = line.cell_range(&link_range)?;
        Some((
            {
                let mut path = trimmed_line.get(path_range)?.to_string();
    ...
    for regex in path_hyperlink_regexes {
            ...
            .map(|captures| (0usize, captures))
        {
            ...
            let match_range = whole_match.range();
            ...
            // trailing `)` from URLs.
            if let Some(trim) =
                first_unbalanced_open_paren(trimmed_line.get(path_range.clone()).unwrap_or_default())
            {
            ...
            let Some(found) = found_from_range(path_range, link_range, line_column) else {
                continue;
            };
...
/// One extracted logical line under the hover point: cell-accurate text
/// (exactly one char per non-spacer cell — a never-written cell reads as a
/// space and a grapheme cluster contributes only its base char, both
/// exactly as alacritty's `cell.c` presents them to the regexes), a
/// byte-offset↔cell span table over it, and the hovered cell's byte offset
/// with alacritty's semantics (a hovered spacer resolves to the last
/// emitted cell's offset).
struct HoverLine {
    text: String,
    spans: Vec<CellSpan>,
    hovered_offset: usize,
}
...
impl HoverLine {
    /// Resolve a non-empty byte range of `text` to the inclusive cell range
    /// its emitting cells cover. A range ending on a wide char covers its
    /// trailing spacer, matching the match ranges alacritty's
    /// `expand_wide`/`sub` dance produces.
    fn cell_range(&self, bytes: &StdRange<usize>) -> Option<(Point, Point)> {
...
fn extract_hover_line(
    backend: &TerminalBackend,
    line_start: Point,
    line_end: Point,
    hovered: Point,
) -> Option<HoverLine> {
    match try_extract_hover_line(backend, line_start, line_end, hovered) {
        Ok(line) => line,
        Err(error) => {
            log::error!("ghostty hover failed to extract the logical line: {error}");
            None
        }
    }
}

/// The ghostty replacement for alacritty's cell walk in `path_match` (which
/// existed because `bounds_to_string` compresses tabs — hover needs a cell
/// accurate string). Tab jumps leave never-written gap cells here, each
/// emitting one space, so the string stays cell accurate.
fn try_extract_hover_line(
    backend: &TerminalBackend,
    line_start: Point,
    line_end: Point,
    hovered: Point,
) -> Result<Option<HoverLine>, GhosttyError> {
    debug_assert!(line_start <= hovered);
    debug_assert!(line_end >= hovered);

    let columns = GridDimensions::of(backend).columns;
    let line_count = (line_end.line - line_start.line).max(0) as usize + 1;
    let mut text = String::with_capacity(line_count * columns);
    ...
    for line in line_start.line..=line_end.line {
        for column in 0..columns {
            ...
            let grid_ref = backend
                .terminal
                .grid_ref(backend.ghostty_screen_point(point))?;
            let cell = grid_ref.cell()?;
            match cell.wide()? {
                CellWide::SpacerTail => {
                    // The spacer extends its wide char's span so byte ranges
                    // ending on the wide char cover both columns.
                ...
                CellWide::Narrow | CellWide::Wide => {
                    prev_len = text.len();
                    let character = if cell.has_text()? {
                        char::from_u32(cell.codepoint()?).unwrap_or(' ')
                    } else {
                        ' '
                    };
                    match character {
                        // Alacritty renders tab cells as plain spaces in
                        // its hover line; keep parity should a cell ever
                        // hold one.
                        ' ' | '\t' | '\0' => text.push(' '),
    ...
    Ok(hovered_offset.map(|hovered_offset| HoverLine {
        text,
        spans,
        hovered_offset,
    }))
...
/// Find the beginning of the current line across linewraps (alacritty
/// `line_search_left`, bounded by [`MAX_SEARCH_LINES`]).
fn line_search_left(backend: &TerminalBackend, mut point: Point) -> Point {
    let topmost_line = GridDimensions::of(backend).topmost_line;
    let limit = point.line.saturating_sub(MAX_SEARCH_LINES as i32);
    while point.line > topmost_line
        && point.line > limit
        && row_is_wrapped(backend, point.line - 1)
    {
        point.line -= 1;
    }
...
/// Find the end of the current line across linewraps (alacritty
/// `line_search_right`, bounded by [`MAX_SEARCH_LINES`]).
fn line_search_right(backend: &TerminalBackend, mut point: Point) -> Point {
    let dimensions = GridDimensions::of(backend);
    let limit = point.line.saturating_add(MAX_SEARCH_LINES as i32);
    while point.line < dimensions.bottommost_line()
        && point.line < limit
        && row_is_wrapped(backend, point.line)
    {
        point.line += 1;
    }
...
fn row_is_wrapped(backend: &TerminalBackend, line: i32) -> bool {
    backend
        .terminal
        .grid_ref(backend.ghostty_screen_point(Point::new(line, 0)))
...
fn hyperlink_uri_at(backend: &TerminalBackend, point: Point) -> Option<String> {
    let grid_ref = backend
        .terminal
        .grid_ref(backend.ghostty_screen_point(point))
        .log_err()?;
    ...
    loop {
        match grid_ref.hyperlink_uri(&mut buffer) {
            Ok(0) => return None,
            Ok(written) => {
                return String::from_utf8(buffer.get(..written)?.to_vec()).log_err();
            }
            Err(GhosttyError::OutOfSpace { required }) => buffer.resize(required, 0),
            Err(error) => {
                log::error!("ghostty hover failed to read a hyperlink uri: {error}");
                return None;
            }
        }
    }
...
/// The re-hosted `alacritty/hyperlinks.rs` suite (#35 test plan, SPEC.md §6
/// P6): the scenario expectations are byte-identical to the alacritty
/// originals; only the harness changed — content feeds the ghostty backend
/// as bytes and grid positions come from the seam's cursor queries.
#[cfg(test)]
mod tests {
    ...
    fn test_url_parentheses_sanitization() {
        ...
        for (input, expected) in test_cases {
            let (result, _) = sanitize_url_punctuation(input.to_string(), 0..input.len());
    ...
    macro_rules! test_hyperlink {
        ($($lines:expr),+; $hyperlink_kind:ident) => { {
            use crate::ghostty::hyperlinks::tests::line_cells_count;
            use std::cmp;
        ...
        ($columns:expr; $total_cells:expr; $lines:expr; $hyperlink_kind:ident) => { {
            use crate::ghostty::hyperlinks::tests::{ test_hyperlink, HyperlinkKind };
    ...
    mod path {
        ...
        mod perf {
            use super::super::*;
            use crate::{Scroll, TerminalSettings};
            use settings::{self, Settings, SettingsContent};
            use std::{cell::RefCell, rc::Rc};
            use util_macros::perf;

            /// The ghostty stand-in for the alacritty harness's `mock_term` +
            /// resize: each `\n`-separated content line writes its chars up
            /// to the first `\r` and then a hard newline, reproducing the
            /// grid the alacritty benchmarks see after their reflow.
            fn build_test_term(
                line: &str,
                repeat: usize,
                hover_offset_column: usize,
            ) -> (TerminalBackend, Point) {
                ...
                for text in content.split('\n') {
                    let visible: String = text.chars().take_while(|c| *c != '\r').collect();
                    backend.write(visible.as_bytes());
                    backend.write(b"\r\n");
                }
                backend.scroll_display(Scroll::Top);
                let point = Point::new(
                    -(backend.scrollback_lines() as i32) + 3,
                    hover_offset_column,
                );
                (backend, point)
            }
            ...
            pub fn cargo_hyperlink_benchmark() {
                ...
                thread_local! {
                    static TEST_TERM_AND_POINT: (TerminalBackend, Point) =
...
";
    ...
    /// OSC 8 hover has no alacritty-suite equivalent; these pin the ghostty
    /// walk directly, including the G7 URI-equality merge (SPEC.md §9).
    mod osc8 {
        use super::*;

        fn hover(backend: &TerminalBackend, point: Point) -> Option<HyperlinkMatch> {
            let mut regex_searches = RegexSearches::default();
            backend.find_from_terminal_point(point, &mut regex_searches, PathStyle::local())
        }

        #[test]
        fn hover_returns_uri_and_extent() {
            let (mut backend, _events_rx) = test_backend(20, 5, 0);
            backend.write(b"pre \x1b]8;;https://example.com\x1b\\LINK\x1b]8;;\x1b\\ post");
            let hyperlink = hover(&backend, Point::new(0, 5)).expect("hyperlink");
            assert!(hyperlink.is_url);
            assert_eq!(hyperlink.text, "https://example.com");
            assert_eq!(
                hyperlink.range,
                Range::new(Point::new(0, 4), Point::new(0, 7))
            );
            assert_eq!(hover(&backend, Point::new(0, 2)), None);
        }

        #[test]
        fn hover_walk_crosses_soft_wrapped_rows() {
            let (mut backend, _events_rx) = test_backend(4, 5, 0);
            backend.write(b"\x1b]8;;https://example.com\x1b\\ABCDEF\x1b]8;;\x1b\\");
            let hyperlink = hover(&backend, Point::new(1, 0)).expect("hyperlink");
            assert_eq!(
                hyperlink.range,
        ...
        #[test]
        fn adjacent_distinct_links_with_identical_uris_merge() {
            // The G7 accepted loss: with no OSC 8 id exposed, two adjacent
            // links carrying the same URI read as one hover region.
            let (mut backend, _events_rx) = test_backend(20, 5, 0);
            backend.write(
                b"\x1b]8;id=a;https://example.com\x1b\\AB\x1b]8;;\x1b\\\
                  \x1b]8;id=b;https://example.com\x1b\\CD\x1b]8;;\x1b\\",
            );
            let hyperlink = hover(&backend, Point::new(0, 0)).expect("hyperlink");
            assert_eq!(
                hyperlink.range,
        ...
        #[test]
        fn file_uri_normalizes_to_path() {
            let (mut backend, _events_rx) = test_backend(30, 5, 0);
            backend.write(b"\x1b]8;;file:///test/cool.rs\x1b\\X\x1b]8;;\x1b\\");
            let hyperlink = hover(&backend, Point::new(0, 0)).expect("hyperlink");
            assert!(!hyperlink.is_url);
            assert_eq!(hyperlink.text, "/test/cool.rs");
        }
    }
    ...
    struct ExpectedHyperlink {
        hovered_grid_point: Point,
        hovered_char: char,
        hyperlink_kind: HyperlinkKind,
        iri_or_path: String,
        row: Option<u32>,
        column: Option<u32>,
        hyperlink_match: Range,
    }

    fn char_at(backend: &TerminalBackend, point: Point) -> char {
        backend
            .terminal
            .grid_ref(backend.ghostty_screen_point(point))
    ...
    /// readability.
    fn build_backend_from_test_lines<'a>(
        hyperlink_kind: HyperlinkKind,
        columns: usize,
        screen_lines: usize,
        test_lines: impl Iterator<Item = &'a str>,
    ) -> (TerminalBackend, ExpectedHyperlink) {
        ...
        fn prev_input_point_from_backend(backend: &TerminalBackend) -> Point {
            let mut point = Point::new(
                backend.terminal.cursor_y().expect("cursor_y") as i32,
                backend.terminal.cursor_x().expect("cursor_x") as usize,
            );
            ...
            if !backend
                ...
                .expect("pending wrap")
            {
                point = point_sub(GridDimensions::of(backend), PointBoundary::Grid, point, 1);
            }

            if backend.wide_at(point) == Some(CellWide::SpacerTail) {
                point.column -= 1;
            }
        ...
        fn end_point_from_prev_input_point(
            backend: &TerminalBackend,
            prev_input_point: Point,
        ) -> Point {
            if backend.wide_at(prev_input_point) == Some(CellWide::Wide) {
                point_add(
                    GridDimensions::of(backend),
                    PointBoundary::Grid,
                    prev_input_point,
                    1,
                )
        ...
        fn process_input(backend: &mut TerminalBackend, c: char) {
            let mut buffer = [0u8; 4];
            backend.write(c.encode_utf8(&mut buffer).as_bytes());
        }

        let mut hovered_grid_point: Option<Point> = None;
        let mut hyperlink_match = Range::new(Point::new(0, 0), Point::new(0, 0));
        let mut iri_or_path = String::default();
        let mut row = None;
        let mut column = None;
        let mut prev_input_point = Point::new(0, 0);
        ...
        for text in test_lines {
            ...
            while let Some(c) = chars.next() {
                match c {
                    ...
                    '👈' => {
                        hovered_grid_point = Some(point_add(
                            GridDimensions::of(&backend),
                            PointBoundary::Grid,
                            prev_input_point,
                            1,
                    ...
                    '‹' | '›' => {
                        match_state = match match_state {
                            ...
                            MatchState::Match(start_point) => {
                                hyperlink_match = Range::new(
                                    start_point,
                                    end_point_from_prev_input_point(&backend, prev_input_point),
                                );
                                MatchState::Done
                            }
                            MatchState::Done => {
                    ...
                    _ => {
                        ...
                        if is_windows_abs_path_start {
                            ...
                            process_input(&mut backend, 'C');
                            prev_input_point = prev_input_point_from_backend(&backend);
                            process_input(&mut backend, ':');
                            process_input(&mut backend, c);
                        } else {
                            process_input(&mut backend, c);
                            prev_input_point = prev_input_point_from_backend(&backend);
                        ...
                        if captures_state == CapturesState::PathNextChar {
                            // The capture accumulates from the fed chars; on
                            // the alacritty harness this was a
                            // `bounds_to_string` readback.
                            captures_state = CapturesState::Path(if is_windows_abs_path_start {
                                String::from("C:")
                            } else {
                                String::new()
                            });
                        }
                        if let CapturesState::Path(path) = &mut captures_state {
                            path.push(c);
                        }
            ...
            // CSI E is alacritty's `move_down_and_cr`: down one row, column
            // zero, clamped at the bottom without scrolling.
            backend.write(b"\x1b[E");
        }
        ...
        let hovered_char = char_at(&backend, hovered_grid_point);
        (
            backend,
            ExpectedHyperlink {
                hovered_grid_point,
    ...
    impl<'a> CheckHyperlinkMatch<'a> {
        ...
        fn check_path_with_position_and_match(
            ...
            let format_path_with_position_and_match =
                |path_with_position: &PathWithPosition, hyperlink_match: &Range| {
                    let mut result =
        ...
        fn check_iri_and_match(&self, iri: String, hyperlink_match: &Range) {
            let format_iri_and_match = |iri: &String, hyperlink_match: &Range| {
                format!(
        ...
        fn format_hyperlink_match(hyperlink_match: &Range) -> String {
            format!(
                "({}, {})..=({}, {})",
        ...
        fn format_renderable_content(&self) -> String {
            ...
            for index in 0..columns {
                ...
                if index == self.expected_hyperlink.hovered_grid_point.column {
            ...
            for line in 0..self.backend.screen_lines() as i32 {
                for column in 0..columns {
                    let point = Point::new(line, column);
                    if matches!(
                        self.backend.wide_at(point),
                    ...
                    if column == 0 {
                        let prefix = if line == self.expected_hyperlink.hovered_grid_point.line {
                            '→'
                        } else {
                            ' '
                        };
                        result += &format!("\n{prefix}[{:>3}] ", line.to_string());
                    }

                    match char_at(self.backend, point) {
                        '\t' => result.push(' '),
                        c => result.push(c),
                    }
                }
    ...
    fn test_hyperlink<'a>(
        ...
        let screen_lines = total_cells / columns + 2;
        let (backend, expected_hyperlink) =
            build_backend_from_test_lines(hyperlink_kind, columns, screen_lines, test_lines);
        ...
        let check_hyperlink_match =
            CheckHyperlinkMatch::new(&backend, &expected_hyperlink, source_location);
        match hyperlink_found {
            ...
            Some(hyperlink) => {
                check_hyperlink_match.check_iri_and_match(hyperlink.text, &hyperlink.range);
            }
            None => {
