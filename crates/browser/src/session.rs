...
//! active index) as a JSON blob in Zed's key-value store, under the key
//! established by Glass (`Glass:crates/browser/src/session.rs:7`). Web
//! session state (cookies, logins) is not persisted here — it lives in the
//! engine's on-disk profile (`cef_instance.rs`, `browser_cache` +
