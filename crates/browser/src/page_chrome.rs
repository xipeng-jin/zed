//! Page theme color for tab tinting (M3 ticket #22), ported from
//! `Glass:crates/browser/src/page_chrome.rs`.
//!
//! The render process samples what is actually visible at the top edge of the
//! page (falling back to `<meta name="theme-color">`) and reports the result
//! to the browser process, which surfaces it through the tab-backend seam as
//! [`TabBackendEvent::PageChromeChanged`](crate::tab_backend::TabBackendEvent).
...
/// How the render process arrived at the reported color.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum PageChromeSource {
    SampledTopEdge,
    ThemeColorMeta,
}

/// A page's reported chrome color, used to tint its tab.
#[derive(Debug, Clone, Copy, PartialEq)]
pub struct PageChrome {
    pub color: Hsla,
    pub source: PageChromeSource,
}
...
#[cfg(feature = "cef")]
pub(crate) use engine::{PageChromeRenderProcessHandlerBuilder, extract_page_chrome_from_message};

/// The render-process side: the injected observer script, the V8 bridge it
/// reports through, and the process-message plumbing back to the browser
/// process.
#[cfg(feature = "cef")]
mod engine {
    use super::{PageChrome, parse_page_chrome_payload};
    ...
    use cef::{
        ...
        RenderProcessHandler, V8Context, V8Handler, V8Value, WrapRenderProcessHandler,
        WrapV8Handler, process_message_create, v8_context_get_current_context,
        v8_value_create_function, wrap_render_process_handler, wrap_v8_handler,
    };

    const PAGE_CHROME_MESSAGE_NAME: &str = "zed.page_chrome";
    const PAGE_CHROME_BRIDGE_NAME: &str = "__zedReportChromeColor";
...
(function () {
  if (window.__zedChromeSyncInstalled) return;
  window.__zedChromeSyncInstalled = true;

  const bridge = window.__zedReportChromeColor;
...
"#;
    ...
    /// Decode a `zed.page_chrome` process message in the browser process.
    /// Returns `None` for unrelated messages, `Some(None)` when the page
    /// reported that no color could be resolved.
        ...
        impl RenderProcessHandler {
            fn on_context_created(
                ...
                if context.eval(
                    ...
                    Some(&CefString::from("zed://page_chrome.js")),
                    0,
                    Some(&mut result),
                ...
                {
                    log::warn!(
                        "[browser::page_chrome] failed to install the page chrome observer: {}",
                        CefString::from(&eval_exception.message())
                    );
                }
            }
